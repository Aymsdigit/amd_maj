                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="post" class="needs-validation" name="versement-form"
                                                id="form-versement" novalidate>
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button id="btn-save-versement" type="submit"
...
$(document).ready(function() {
    ...
    function load_versement() {
        $.ajax({
            url: "versement/fetch_versement",
            method: 'POST',
            dataType: "JSON",
    ...
    $('#btn-new-versement').click(function() {
        $('#form-versement')[0].reset();
        $('#motif-error').text('');
        $('#montant-error').text('');
        $('#modal-title').text('Nouveau versement');
        $('#action').val('Add');
        $('#btn-save-versement').text('Ajouter');
        $('#myModal').modal('show');
    });

    $('#form-versement').on('submit', function(event) {
        event.preventDefault();

        $.ajax({
            url: "versement/insert",
            method: "POST",
            data: $(this).serialize(),
            dataType: "JSON",

            beforeSend: function() {
                $('#btn-save-versement').text('patientez...');
                $('#btn-save-versement').attr('disabled', 'disabled');
            },

            success: function(data) {
                $('#btn-save-versement').text('Save');
                $('#btn-save-versement').attr('disabled', false);
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#modal-title').text('Modification de versement : ' + data.motif);
                ...
                $('#btn-save-versement').text('Mettre à jour');
                $('#myModal').modal('show');
                $('#hidden_id').val(id);
            }
        })
